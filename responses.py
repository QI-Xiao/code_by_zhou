...
class SUCCESS(RestResponse):
    """
    此类实例化过程必须提供一个data属性作为成功的数据字典，
    若extra_dict中也有重名的data键值对，将会被前者覆盖。
    ...
    def _check_set(self):
        if not self._has_code:
            raise RestResponserHandlerError(
                "you have not set a SUCCESS code yet.")
    ...
    def set_status(cls, status):
        if not isinstance(status, str):
            raise TypeError(
                f"the type of status must be str, current is: {type(status)}."
            )
        cls._status = status
    ...
    def set_code(cls, code):
        if not isinstance(code, int):
            raise TypeError(
                f"the type of code must be int, current is: '{type(code)}'."
            )
        cls._code = code
        cls._has_code = True
...
class FAIL(RestResponse):
    ...
    def _check_set(self):
        if not self._has_code_map:
            raise RestResponserHandlerError(
                "you have not set a FAIL code_map yet."
            )
    ...
    def _add_why(self, code, extra_dict):
        ...
        if why is None:
            raise RestResponserHandlerError(
                f"the error code '{code}' is not found in code_map."
            )
        extra_dict['why'] = why
    ...
    def set_status(cls, status):
        if not isinstance(status, str):
            raise TypeError(
                f"the type of status must be str, "
                f"current is: '{type(status)}'"
            )
        cls._status = status
    ...
    def set_code_map(cls, code_map):
        def check():
            if not isinstance(code_map, dict):
                raise TypeError(
                    f"the type of code_map must be dict, "
                    f"current is: '{type(code_map)}'"
                )
            ...
            for code, why in code_map.items():
                if not isinstance(code, int):
                    raise TypeError(
                        f"the type of code must be int, "
                        f"current is: '{type(code)}'."
                    )

                if not isinstance(why, str):
                    raise TypeError(
                        f"the type of why must be int, "
                        f"current is: '{type(why)}'."
                    )
